...
# Configuration
REPO_URL_SSH="git@github.com:cxhihilwb123-hash/jujing-agent.git"
REPO_URL_HTTPS="https://github.com/cxhihilwb123-hash/jujing-agent.git"
HERMES_HOME="${JUJING_HOME:-$HOME/.jujing-agent}"
...
#   data still at /root/.jujing-agent (HERMES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.jujing-agent/hermes-agent"
            echo "                   default (root, Linux): /usr/local/lib/hermes-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.jujing-agent, or \$JUJING_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, 巨鲸智能体 installs the code under"
            echo "  /usr/local/lib/hermes-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.jujing-agent).  This keeps Docker bind-mounted volumes"
...
# Keep the Jujing-branded home visible to child processes while still exporting
# HERMES_HOME for the upstream kernel contract.
export JUJING_HOME="$HERMES_HOME"
export HERMES_HOME
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"检查系统环境","category":"runtime","needs_user_input":false},{"name":"repository","title":"下载巨鲸智能体运行环境","category":"runtime","needs_user_input":false},{"name":"venv","title":"创建 Python 虚拟环境","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"安装 Python 依赖","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"安装浏览器工具依赖","category":"runtime","needs_user_input":false},{"name":"path","title":"安装命令行入口","category":"runtime","needs_user_input":false},{"name":"config","title":"准备配置与技能","category":"configuration","needs_user_input":false},{"name":"setup","title":"配置账号与模型","category":"configuration","needs_user_input":true},{"name":"gateway","title":"配置网关服务","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"完成安装","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  请使用巨鲸智能体 Windows 安装包，或运行随包提供的 install.ps1"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. 巨鲸智能体 install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "巨鲸智能体 itself does not require or retain root access."
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# 巨鲸智能体 — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# 巨鲸智能体 — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# 巨鲸智能体 — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create the product home directory structure (config at top level, code in subdir)
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
	    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# 巨鲸智能体 browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
	        log_info "巨鲸智能体 will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# 巨鲸智能体 browser tools — explicit browser override."
