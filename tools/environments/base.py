...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # by ``&``-launched subshells, while ``$BASHPID`` is absent from the
        # Bash 3.2 shipped by macOS. Use mktemp so uniqueness is portable across
        # both shells and the temp stays beside the destination for atomic mv.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file. mktemp works with macOS Bash 3.2 (which
        # has no BASHPID) and creates a unique sibling on the same filesystem.
        _snap_template = self._quote_shell_path(
            self._snapshot_path + ".tmp.XXXXXX"
        )
