...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, '巨鲸智能体.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', '巨鲸智能体'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, '巨鲸智能体.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, '巨鲸智能体'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Jujing-Agent-${PACKAGE_JSON.version}-mac-${ARCH}.dmg`)
  }

  const prefix = `Jujing-Agent-${PACKAGE_JSON.version}-mac-`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Jujing-Agent-${PACKAGE_JSON.version}-mac-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Jujing-Agent-${version}-${os}-${arch}.${ext}'
