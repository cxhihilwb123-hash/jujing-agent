#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Jujing icon + version metadata onto the
// built Windows exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Windows exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-exe>
//
...
// Stamp the Jujing icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Jujing Agent',
      FileDescription: 'Jujing Agent',
      CompanyName: 'Jujing Network',
      LegalCopyright: 'Copyright (c) 2026 Jujing Network'
    }
  })

  console.log('[set-exe-identity] done — Jujing Agent icon + identity stamped')
}
