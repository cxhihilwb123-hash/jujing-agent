...
export function Backdrop() {
  ...
  const brandBackdrop = useControls(
    'Backdrop / Brand',
    {
  ...
  return (
    <>
      ...
      {on && brandBackdrop.enabled && (
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 z-2"
          style={{
            mixBlendMode: brandBackdrop.blendMode as CSSProperties['mixBlendMode'],
            opacity: brandBackdrop.opacity
          }}
        >
          <img
            alt=""
            ...
            style={{
              height: `${brandBackdrop.scale}dvh`,
              objectPosition: brandBackdrop.objectPosition,
              filter: `invert(calc(${brandBackdrop.invert ? 1 : 0} * var(--backdrop-invert-mul, 1))) saturate(${brandBackdrop.saturate}) brightness(${brandBackdrop.brightness})`
            }}
          />
        </div>
      )}
