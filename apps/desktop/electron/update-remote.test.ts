 ...
 * origin=git@github.com:cxhihilwb123-hash/jujing-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:CXHIHILWB123-HASH/JUJING-AGENT.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:cxhihilwb123-hash/jujing-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/cxhihilwb123-hash/jujing-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/cxhihilwb123-hash/jujing-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:cxhihilwb123-hash/jujing-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/cxhihilwb123-hash/jujing-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:cxhihilwb123-hash/jujing-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/cxhihilwb123-hash/jujing-agent.git'), true)
  assert.equal(isSshRemote('https://github.com/cxhihilwb123-hash/jujing-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:CXHIHILWB123-HASH/JUJING-AGENT.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:cxhihilwb123-hash/jujing-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/cxhihilwb123-hash/jujing-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:someuser/jujing-agent.git'), false)
  // Same repo name on a different host is not the official repo.
  assert.equal(isOfficialSshRemote('git@gitlab.com:cxhihilwb123-hash/jujing-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/cxhihilwb123-hash/jujing-agent.git'), false)
