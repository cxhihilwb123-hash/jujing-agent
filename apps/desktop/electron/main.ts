...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[jujing] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[jujing] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[jujing] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
...
if (INSTALL_STAMP) {
  console.log(
    `[jujing] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[jujing] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
function normalizeSoulText(text) {
  return String(text || '')
    .replace(/\r\n/g, '\n')
    .replace(/\r/g, '\n')
    .replace(/^\uFEFF/, '')
...
const JUJING_SOUL_MD =
  '你是巨鲸智能体，由巨鲸网络打造的智能 AI 助手。你专业、可靠、直接，面向企业经营、增长、销售、运营和管理场景提供帮助。' +
  '你能够拆解问题、寻找客户线索、规划获客动作、协助账号运营、整理资料、生成内容，并推动流程执行。' +
  '你会清晰表达，在不确定时主动说明，并优先产出可落地、可推进、可复用的业务结果。除非用户特别要求展开，否则保持聚焦、高效，先理解业务目标，再给出可执行的帮助。'

function ensureJujingSoul(hermesHome = HERMES_HOME) {
  ...
  try {
    ...
    try {
      current = fs.readFileSync(soulPath, 'utf8')
    } catch (error) {
    ...
    const normalized = normalizeSoulText(current)

    if (!normalized || normalized === normalizeSoulText(DEFAULT_HERMES_SOUL_MD)) {
      fs.writeFileSync(soulPath, JUJING_SOUL_MD + '\n', 'utf8')
      rememberLog(`[branding] wrote 巨鲸智能体 SOUL.md at ${soulPath}`)
    }
  } catch (error) {
    rememberLog(`[branding] failed to ensure 巨鲸智能体 SOUL.md: ${error.message}`)
  }
}
...
// electron-builder derives from the build `appId` (com.jujing.network.agent) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.jujing.network.agent')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Jujing Network'
})
...
const MEDIA_PROTOCOL = 'jujing-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(jujing-media://…)`.
...
function findGitBash() {
  ...
  if (fromPath) {
    process.env.HERMES_GIT_BASH_PATH = fromPath
  }
...
// applyUpdates — Jujing-branded builds update the agent runtime only.
//
// The desktop shell is a controlled product surface, so the in-app update button
// must not rebuild or replace 巨鲸智能体.app from the upstream checkout. Desktop
// app upgrades are distributed as signed DMGs; this flow only moves the local
// CLI/backend runtime forward and tells the user to restart.
async function applyUpdates(opts = {}) {
  ...
  try {
    return await applyKernelOnlyUpdate(opts)
  } finally {
    updateInFlight = false
  }
}

async function applyKernelOnlyUpdate(_opts: any = {}) {
  const updateRoot = resolveUpdateRoot()
  const hermes = resolveHermesCliBinary(updateRoot)

  if (!hermes) {
    const command = 'hermes update'
    rememberLog(`[updates] no runtime CLI found; surfacing manual \`${command}\` for ${updateRoot}`)
    emitUpdateProgress({ stage: 'manual', message: command, percent: null })

    return { ok: true, manual: true, command, hermesRoot: updateRoot, kernelOnly: true }
  }
  ...
  try {
    ...
    // Best-effort: a detached or non-git install can still use the CLI default.
  }
  ...
  // Windows locks the venv shim while the backend is alive; release it before
  // the runtime update. POSIX keeps the existing backend running and the new
  // runtime takes effect after restart.
  const lock = await releaseBackendLockForUpdate(updateRoot)

  if (!lock.unlocked) {
    emitUpdateProgress({
      stage: 'error',
      message: '巨鲸智能体内核仍在运行，无法安全更新。请关闭其他巨鲸智能体或 Hermes 进程后重试。',
      error: 'runtime-locked'
    })

    return { ok: false, error: 'runtime is locked', kernelOnly: true }
  }

  emitUpdateProgress({
    stage: 'update',
    message: '正在更新巨鲸智能体内核，桌面应用保持不变…',
    percent: 10
  })
  ...
  if (updated.code !== 0) {
    ...
    return { ok: false, error: 'kernel update failed', kernelOnly: true }
  }

  emitUpdateProgress({
    stage: 'done',
    message: '巨鲸智能体内核已更新，桌面应用和品牌包装保持不变。重启后生效。',
    percent: 100
  })

  return { ok: true, backendUpdated: true, guiUpdated: false, kernelOnly: true, manualRestart: true }
}
...
function runStreamedUpdate(command, args, { cwd, env, stage }: any = {}) {
  return new Promise<{ code: number; error?: string }>(resolve => {
    ...
    try {
      ...
      resolve({ code: 1, error: err instanceof Error ? err.message : String(err) })
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no agent runtime found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        '巨鲸智能体恢复流程已交给安装器处理。恢复完成后桌面端会自动重启。'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      packagedResourcesPath: IS_PACKAGED ? process.resourcesPath : null,
      hermesHome: HERMES_HOME,
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('巨鲸智能体安装已取消。') as Error & {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `巨鲸智能体初始化失败${bootstrapResult.failedStage ? `，阶段：'${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `巨鲸智能体运行环境 ${ACTIVE_HERMES_ROOT} 缺失或不完整。` + '请通过桌面安装器或 scripts/install.ps1 重新安装。'
    )
  }
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      '巨鲸智能体在 Windows 上需要 Git for Windows（提供 Git Bash，' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `巨鲸智能体 Python 环境缺失：${VENV_ROOT}。请重新运行桌面安装器或 ` + '`scripts/install.ps1` 进行修复。'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `巨鲸智能体运行环境 ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: '巨鲸智能体运行环境已就绪',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported agent service URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the agent service.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to agent service after ${timeoutMs}ms`))
    })
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`巨鲸智能体服务未能就绪：${lastError?.message || 'timeout'}`)
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? '正在连接巨鲸云智能体…' : '登录巨鲸智能体网关',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to agent service after ${timeoutMs}ms`))
    }, timeoutMs)
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Jujing Agent Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Jujing Agent Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Jujing Agent Cloud. Open Settings → Gateway, choose Jujing Agent Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error(
        'Your Jujing Agent Cloud session has expired. Open Settings → Gateway and sign in again.'
      ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Jujing Agent Cloud session has expired. Sign in to Jujing Agent Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        '远程智能体网关使用 OAuth，但当前尚未登录。' + '请打开“设置 → 网关”并点击“登录”，或切回本地模式。'
      ) as any
  ...
  if (!token) {
    throw new Error('已选择远程智能体网关，但尚未保存会话令牌。' + '请打开“设置 → 网关”保存令牌，或切回本地模式。')
  }
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Jujing Agent service.'
      )
    }
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting agent service for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Agent service for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Agent service for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Agent service for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `agent service for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', '正在解析巨鲸智能体服务', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `正在连接远程巨鲸智能体服务：${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: '远程巨鲸智能体服务已就绪',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.runtime', '正在解析巨鲸智能体运行环境', 28)
    const backend = await ensureRuntime(resolveHermesBackend(backendArgs))
    ensureJujingSoul()
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    ...
    await advanceBootProgress('backend.spawn', `正在启动巨鲸智能体服务：${backend.label}`, 84)
    rememberLog(`Starting agent service via ${backend.label}`)
    ...
    hermesProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`忽略已失效的巨鲸智能体服务错误：${error.message}`)
        rejectBackendStart?.(new Error('Hermes backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`巨鲸智能体服务启动失败：${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `巨鲸智能体服务启动失败：${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`忽略已失效的巨鲸智能体服务退出事件（${signal || code}）`)
      ...
      rememberLog(`巨鲸智能体服务已退出（${signal || code}）`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `巨鲸智能体服务尚未就绪就已退出（${signal || code}）。`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `巨鲸智能体服务尚未就绪就已退出（${signal || code}）。日志：${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', '正在等待巨鲸智能体服务启动', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', '正在等待巨鲸智能体服务就绪', 90)
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('缓存的远程巨鲸智能体服务未通过存活检测，正在丢弃旧连接。')
