...
test('resolveInstallScript prefers a packaged installer resource', async () => {
  const home = mkTmpHome()
  const resources = fs.mkdtempSync(path.join(os.tmpdir(), 'jujing-resources-test-'))

  try {
    const scriptsDir = path.join(resources, 'scripts')
    fs.mkdirSync(scriptsDir, { recursive: true })
    const packaged = path.join(scriptsDir, SCRIPT_NAME)
    fs.writeFileSync(packaged, '#!/bin/sh\necho packaged\n')

    assert.equal(resolvePackagedInstallScript(resources), packaged)
    ...
    const result = await resolveInstallScript({
      ...
      _download: async () => {
        throw new Error('network should not be used for packaged installer')
      }
    })

    assert.equal(result.source, 'packaged')
    assert.equal(result.path, packaged)
    assert.ok(logs.some(ev => /using packaged/.test(ev.line || '')))
  } finally {
    fs.rmSync(home, { recursive: true, force: true })
