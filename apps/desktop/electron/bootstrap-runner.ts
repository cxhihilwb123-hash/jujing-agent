...
function resolvePackagedInstallScript(resourcesPath = process.resourcesPath) {
  if (!resourcesPath) {
    return null
  }

  const candidate = path.join(resourcesPath, 'scripts', installScriptName())
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/cxhihilwb123-hash/jujing-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  // 1. Packaged apps carry the branded installer scripts as extra resources.
  //    Prefer those so first-launch setup does not have to download the
  //    installer itself before the UI can show progress.
  const packagedScript = resolvePackagedInstallScript(packagedResourcesPath)
  ...
  // 2. Dev shortcut: prefer a local checkout's installer so we can iterate
  ...
  // 3. Download from the Jujing repository at the install stamp's ref.
