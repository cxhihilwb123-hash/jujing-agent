...
async function applyKernelOnlyUpdate() {
  const updateRoot = resolveUpdateRoot()
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('巨鲸智能体安装已取消。')
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `巨鲸智能体运行环境 ${ACTIVE_HERMES_ROOT} 缺失或不完整。` +
        '请通过桌面安装器或 scripts/install.ps1 重新安装。'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `巨鲸智能体 Python 环境缺失：${VENV_ROOT}。请重新运行桌面安装器或 ` +
        '`scripts/install.ps1` 进行修复。'
    )
  }
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        '远程智能体网关使用 OAuth，但当前尚未登录。' +
          '请打开“设置 → 网关”并点击“登录”，或切回本地模式。'
      )
  ...
  if (!token) {
    throw new Error(
      '已选择远程智能体网关，但尚未保存会话令牌。' +
        '请打开“设置 → 网关”保存令牌，或切回本地模式。'
    )
  }
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Agent service failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`Agent service exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
...
ipcMain.handle('hermes:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('Cached remote agent service failed liveness probe; dropping stale connection.')
    resetHermesConnection()
