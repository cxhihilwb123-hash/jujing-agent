...
profile is active, get_hermes_home() should:
  1. STILL return ~/.jujing-agent (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.jujing-agent."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".jujing-agent"
        assert "HERMES_HOME fallback" not in capsys.readouterr().err

    def test_default_active_profile_no_warning(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=default → still no warning, returns ~/.jujing-agent."""
        hermes_dir = tmp_path / ".jujing-agent"
        hermes_dir.mkdir()
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".jujing-agent" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".jujing-agent" / "active_profile").write_text("coder\n")
