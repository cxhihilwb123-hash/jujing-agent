...
def _hermes_home_from_env() -> Path:
    ...
    Reads ``JUJING_HOME`` first and ``HERMES_HOME`` as the internal upstream
    compatibility variable, then falls back to the platform-native
...
def get_hermes_home() -> Path:
    ...
    :func:`set_hermes_home_override`) → ``JUJING_HOME`` env var →
    ``HERMES_HOME`` compatibility env var → the platform-native default.
    ...
    if not (os.environ.get("JUJING_HOME", "").strip() or os.environ.get("HERMES_HOME", "").strip()):
